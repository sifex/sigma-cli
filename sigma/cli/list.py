...
def list_formats(backend):
    ...
    table.add_rows(
        [(name, description) for name, description in backends[backend].formats.items()]
    )
    table.align = "l"
    click.echo(table.get_string())
...
@click.argument("backend", required=False, type=click.Choice(backends.keys()))
def list_pipelines(backend):
    ...
    for name, definition in pipelines.items():
        if (
            backend is None
            or backend in definition.backends
            or len(definition.backends) == 0
        ):
